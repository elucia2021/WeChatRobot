...
class Robot(Job):
    ...
    def toAt(self, msg: WxMsg) -> bool:
        ...
        """
        new_content = re.sub(r"@.*?[\u2005|\s]", "", msg.content)
    ...
    def toPrivatechat(self, msg: WxMsg) -> bool:
        """闲聊，接入 ChatGPT
        """
        if not self.chat:  # 没接 ChatGPT，固定回复
            return True    #不回复
    ...
    def processMsg(self, msg: WxMsg) -> None:
        ...
        if msg.type == 37:  # 好友请求
            ...
            # 让配置加载更灵活，自己可以更新配置，给跨端给自己账号（手机发电脑、ipad发电脑）发送更新指令即可。也可以利用定时任务更新。
            #print("文本信息")
            #print(msg.from_self())
            if msg.from_self():
                #print("更新判定结果", msg.content == "更新")
                if msg.content == "更新":
                    self.config.reload()
                    self.LOG.info("已更新")
            else:  #非群聊信息，单独调用一个私聊函数
                self.toPrivatechat(msg)  # 闲聊
